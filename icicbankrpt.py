...
if fl is not None:
    ...
    with st.expander("Top 5 Withdrawls"):
       ...
       st.bar_chart(top_5_withdrawals[['Value Date', 'Withdrawals']], color=["#0000FF"]).set_index('Value Date')  
    ...
    if len(selected_key) > 0:
      ...
      st.bar_chart(choice_data[['Value Date', 'Withdrawals', 'Deposits', 'Matched_Keyword']], color="Matched_Keyword").set_index('Value Date')   
    #
